...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
	float sum_up = 0;
	vector<float> new_row; // new row

	// sum up the all elements
	...
	// all elements to be devided by "sum_up"
	for (int i = 0; i < grid.size(); i++) {
		new_row.clear();
		for (int j = 0; j < grid[0].size(); j++) {
			new_row.push_back(grid[i][j] / sum_up);
		}
		newGrid.push_back(new_row);
	}

	cout << "normalize function being tested !\n";
	return newGrid;
}
...
//	vector < vector <float> > newGrid; define and initialize the "newGrid" at the same time
	...
	//define and initialize the "newGrid" at the same time
	vector < vector <float> > newGrid(height, vector<float>(width, 0.0)); 

	// form a matrix of coefficient
	center = 1.0 - blurring;
	...
	for (int i = 0; i < 3; i++) {
		row.clear();
		for (int j = 0; j < 3; j++) {
			if (((i == 0) || (i == 2)) && ((j == 0) || (j == 2)))	{ val = corner; }
			else if ((i == 1) && (j == 1))							{ val = center; }
			else													{ val = adjacent; }
			row.push_back(val);
		}
	...
	// Multiply coefficient for each elements of grid
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			for (int k = -1; k < 2; k++) {
				for (int l = -1; l < 2; l++) {
					multi = window[k+1][l+1];
					new_i = (i + k + height) % height;
					new_j = (j + l + width) % width;
					newGrid[new_i][new_j] += multi * grid[i][j];
				}
			}
		}
	}
	cout << "blur function being tested !\n";
